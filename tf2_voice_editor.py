...
import sys
import os
# --- START PYTORCH/QT DLL FIX ---
# This fix resolves the WinError 1114 by pre-loading PyTorch DLLs before Qt initialization.
# It MUST be placed before other imports that use these libraries (like PyQt6 or torch).
import platform
import ctypes
from pathlib import Path

if platform.system() == "Windows" and getattr(sys, 'frozen', False):
    # sys._MEIPASS is the path to the temp folder in PyInstaller onefile mode
    try:
        torch_lib_path = Path(sys._MEIPASS) / "torch" / "lib"
        if torch_lib_path.exists():
            # Pre-load the necessary DLLs to satisfy dependencies
        ...
        # Fail gracefully if path resolution fails
        pass
# --- END PYTORCH/QT DLL FIX ---
